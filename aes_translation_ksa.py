...
class AESBlockCipherHuang(Cipher):
    """
    ...
    def __init__(self, number_of_rounds=10, word_size=8, state_size=4, key_size=-1):
        ...
        super().__init__(family_name="aes_block_cipher_Huang",
        ...
        self.transposition_box = [0,4,8,12,1,5,9,13,2,6,10,14,3,7,11,15]
        ...
        # if self.KEY_BLOCK_SIZE == 256:
        #     for round_number in range(number_of_rounds):
        #         sboxes_components = self.create_sbox_components(add_round_key, first_add_round_key, round_number, word_size)
        #         shift_row_components = self.create_shift_row_components(sboxes_components, word_size)
        #         mix_column_components = self.create_mix_column_components(round_number, shift_row_components, word_size)
        #         if round_number%2 != 0:
        #             tmp_round_number = round_number-1
        #             key_rotation = self.create_rotate_component(remaining_xors, tmp_round_number, word_size)
        #             key_sboxes_components = self.create_key_sbox_components(key_rotation, word_size)
        #             constant = self.create_constant_component(round_number//2, state_size, word_size)
        #             remaining_xors_2, xor1 = self.create_xor_components(constant, key_sboxes_components,
        #                                                       remaining_xors, xor1, tmp_round_number, False)
        #             key_sboxes_components = self.create_key_sbox_components(remaining_xors_2[3], word_size)
        #             remaining_xors_3, xor2 = self.create_xor_components(constant, key_sboxes_components,
        #                                                       remaining_xors, xor2, tmp_round_number, True)
        #             remaining_xors_2.extend(remaining_xors_3)
        #             remaining_xors = remaining_xors_2
        #             updated_key = self.add_intermediate_output_component([remaining_xors[i].id for i in range(8)],
        #                                                    [[i for i in range(self.ROW_SIZE)] for _ in range(2*self.NUM_ROWS)],
        #                                                    256,
        #                                                    "updated_key")
        #             self.add_intermediate_output_component([updated_key.id for i in range(4)],
        #                                            [[i for i in range(0,32)]]+[[i for i in range(32,64)]]+
        #                                            [[i for i in range(64,96)]]+[[i for i in range(96,128)]],
        #                                            128,
        #                                            "round_key_output")
        #             add_round_key = self.create_round_key(mix_column_components, [remaining_xors[i] for i in range(4)],
        #                                           round_number, shift_row_components)
        #             self.create_round_output_component(add_round_key, number_of_rounds, round_number)
                # elif round_number==0 :
                #     self.add_intermediate_output_component([INPUT_KEY for i in range(self.NUM_ROWS)],
                #                                     [[i for i in range(128,160)]]+[[i for i in range(160,192)]]+
                #                                     [[i for i in range(192,224)]]+[[i for i in range(224,256)]],
                #                                    128,
                #                                    "round_key_output")
                #     add_round_key = self.create_round_key(mix_column_components, [INPUT_KEY],
                #                                   round_number, shift_row_components)
                #     self.create_round_output_component(add_round_key, number_of_rounds, round_number)
                # else :
                #     self.add_intermediate_output_component([updated_key.id for i in range(4)],
                #                                    [[i for i in range(128,160)]]+[[i for i in range(160,196)]]+
                #                                    [[i for i in range(196,224)]]+[[i for i in range(224,256)]],
                #                                    128,
                #                                    "round_key_output")
                #     add_round_key = self.create_round_key(mix_column_components, [remaining_xors[i] for i in range(4,8)],
                #                                   round_number, shift_row_components)
                #     self.create_round_output_component(add_round_key, number_of_rounds, round_number)
        ...
        for round_number in range(number_of_rounds):
                ...
                updated_key = self.add_intermediate_output_component([remaining_xors[i].id for i in range(self.NUM_ROWS)],
                                                   [[i for i in range(self.ROW_SIZE)] for _ in range(self.NUM_ROWS)],
                                                   self.KEY_BLOCK_SIZE,
                                                   "updated_key")
                round_key = self.transposition_key_matrix(updated_key, word_size)
                add_round_key = self.create_round_key(mix_column_components, round_key,
                                                  round_number, shift_row_components)
                self.create_round_output_component(add_round_key, number_of_rounds, round_number)
    ...
    def create_xor_components(self, constant, key_sboxes_components, remaining_xors, xor1, round_number, successor=False):
        ...
        if self.KEY_BLOCK_SIZE == 256 and successor == True:
            ...
            [key_sboxes_components[i].id for i in range(self.NUM_ROWS)] + [constant.id],
            [[i for i in range(self.SBOX_BIT_SIZE)] for _ in range(self.NUM_ROWS)] + [[i for i in range(self.ROW_SIZE)]],
            self.ROW_SIZE).id
            if round_number == 0:
                xor1 = self.add_XOR_component(
                [xor_tmp, INPUT_KEY],
    ...
    def transposition_key_matrix(self, updated_key ,word_size):
        if word_size != 8: 
            raise ValueError("word_size incorrect (should be 8)")
        transposition_order = []
        for i in range(16):
            tmp = [j+self.transposition_box[i]*8 for j in range(8)]
            transposition_order.append(tmp)
        # print(transposition_order)
        
        # print(permutation_order)
        # print(updated_key.id)
        # print([updated_key.id for _ in range(16)])
        key_updated = self.add_intermediate_output_component([updated_key.id for _ in range(16)],
                                                             transposition_order,
                                                         self.KEY_BLOCK_SIZE,
                                                         "round_key_output")
        return key_updated
    ...
    def create_round_key(self, mix_column_components, updated_key, round_number, shift_row_components):
        if round_number != self.NROUNDS - 1:
                add_round_key = self.add_XOR_component(
                [mix_column_components[i].id for i in range(self.NUM_ROWS)] + [updated_key.id],
                [[i for i in range(self.ROW_SIZE)] for _ in range(self.NUM_ROWS)] + [[i for i in range(self.KEY_BLOCK_SIZE)]],
                ...
                for i in range(self.NUM_ROWS):
                    shift_rows_input_position_lists.extend(
                        [[j for j in range(i * self.SBOX_BIT_SIZE, (i + 1) * self.SBOX_BIT_SIZE)] for _ in
                         range(self.NUM_ROWS)])
                add_round_key = self.add_XOR_component(
                    shift_rows_ids + [updated_key.id],
                    shift_rows_input_position_lists + [[i for i in range(self.KEY_BLOCK_SIZE)]],
                    self.CIPHER_BLOCK_SIZE)
