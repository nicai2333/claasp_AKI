...
class Cipher:
    ...
    def key_schedule_algorithm(self):
        ...
        cipher_name = self.id.split('_')[0]
        number_of_rounds = self.number_of_rounds
        
        if cipher_name == 'midori': number_of_rounds-=1
        if cipher_name == 'des':
            key_schedule_instance = Cipher(f"{self.id}{KEY_SCHEDULE_ALGORITHM}", f"{self.type}", ['key'], 
                                       [56], self.inputs_size_to_dict()['key'])
            for index,id in enumerate(key_schedule_component_ids):
                if id =='key': 
                    continue
                if index+1 == len(key_schedule_component_ids):
                    key_schedule_component_ids.remove(id)
                else:
                    component_round_1 = find_first_number(id)
                    component_round_2 = find_first_number(key_schedule_component_ids[index+1])
                    if  component_round_1 != component_round_2:
                        key_schedule_component_ids.remove(id)
        ...
        for ids in key_schedule_component_ids:
            ...
            if ids.split('_')[0] == 'intermediate' :
                # print('x')
                component = self.get_component_from_id(ids)
                # print(component.description)
                if (component.description[0] == 'round_key_output') :
                    if (component.input_id_links[0].split('_')[0]=='intermediate') and (len(component.input_id_links)==1) :
                        continue
                    elif len(component.input_id_links)>1 :
                        flag = True
                        for i in range(len(component.input_id_links)-1):
                            if component.input_id_links[i] != component.input_id_links[i+1]:
                                flag = False
                        if flag == True: 
                            continue
    ...
    def reduce_key_schedule_algorithm_rounds(self,target_round_num):
        ...
        for component in components:
            if int(component.id.split('_')[-2])<target_round_num:
                new_component = Component(component.id, component.type, Input(component.input_bit_size, 
                                                                          component.input_id_links, component.input_bit_positions),
                                    component.output_bit_size, component.description)
                new_component.__class__ = component.__class__
                tmp_ksa_component.append(new_component)
        
        for _ in range(target_round_num):
            # print(_)
            key_schedule_instance.add_round()
        for components in tmp_ksa_component:
            # print(components.id)
            round = int(components.id.split('_')[-2])
            key_schedule_instance._rounds.round_at(round)._components.append(components)
        ...
        key_schedule_instance.remove_round_component_from_id(target_round_num-1,id)
    ...
    def key_schedule_inverse(self):
        ...
        #遍历所有组件，将输入组件的输出和当前组件的输入作为等价bit加入equivalent_bits中
        x = 0 
        y = 0
        cipher_component_2 = cipher_component.copy()
        while len(cipher_component_2) > 0:
            number_of_unprocessed_components = 0
            number_of_c = 0

            for c in cipher_component_2:
                # print("c.id:",c.id)
                if are_there_enough_available_inputs_to_evaluate_component(c, available_bits, all_equivalent_bits,
                                                                           key_schedule_component_ids, self):
                    # print("1:",c,",c.id:",c.id)
                ...
                # OPTION 2 - Add components that are invertible
                                                                                                                     ...
                                                                                                                     self,KSA=True)) or (
                        c.type == CIPHER_INPUT and (c.description[0] == INPUT_KEY or c.description[0] == INPUT_TWEAK)) :
                    if (c.id == 'key') and (len(cipher_component_2)!=1): 
                        number_of_unprocessed_components += 1
                        if number_of_unprocessed_components == len(cipher_component_2):
                            raise Error("Unable to invert cipher for now.")
                        continue
                    # print("2:",c,",id:",c.id)
                    # print()
                    ...
                    update_available_bits_with_component_output_bits(c, available_bits, self)
                    # if c.id == 'xor_1_55': 
                    #     print(all_equivalent_bits['xor_1_55_0_output_updated'])
                    inverted_KSA_components.append(inverted_component)
                    cipher_component.remove(c)
    ...
    def correlated_key_schedule(self, ksa_type):
            ...
            for index,component in enumerate(key_schedule_component):
                if (component.type == SBOX) or (component.type == WORD_OPERATION and component.description[0]=="NOT"):
                    ...
                    for c in output_components:
                        for index, link_component in enumerate(c.input_id_links):
                            if link_component == id:
                                for output_component_of_link in key_schedule_component:
                                    if output_component_of_link.id == c.id:
                                        break
                                new_input_id_link = output_component_of_link.input_id_links.copy()
                                new_input_id_link[index] = component.input_id_links[0]
                                new_input_bit_positions = output_component_of_link.input_bit_positions.copy()
                                new_input_bit_positions[index] = component.input_bit_positions[0]

                                new_component = Component(c.id, c.type, Input(c.input_bit_size, 
                                                        new_input_id_link, new_input_bit_positions),
                                        c.output_bit_size, c.description)
                                new_component.__class__ = component.__class__
                                setattr(new_component, "round", getattr(component, "round"))

                                key_schedule_component.remove(output_component_of_link)
                                key_schedule_component.append(new_component)

        for component in key_schedule_component:
            if component.id == 'key' : 
                new_component = Component(component.id, component.type, Input(component.input_bit_size, 
                                                    component.input_id_links, component.input_bit_positions),
                                    component.output_bit_size, component.description)
        ...
        for _ in range(self.number_of_rounds):
            key_schedule_instance.add_round()
        for component in corrlated_key_schedule_components:
